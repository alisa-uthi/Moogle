		...
		else 
		{
			...
			switch (command)
			{
				...
				case "search+":	
					...
					{	
						// the total number of parameters should >= 5 and must be odd number 
						System.out.println("Error: See Usage");
						printMainUsage = true;
					} 
					else 
			...
			while(!done)
			{
				...
				switch(opt.charAt(0)) 
				{
					...
					case 'S':	
						...
						if(keys.length < 3 || keys.length % 2 == 0) 
						{	
							// the total number of parameters should >= 3 and must be odd number 
							System.out.println("Error: See Usage");
							printSearchUsage();
						}
						else 
					...
					default:
						System.out.println("Error: Incorrect option, please see usage");
						break;
				}
				if(!done)
					System.out.println(nextUsage);
