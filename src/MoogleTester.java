...
public class MoogleTester 
{
	...
	public static void SampleSearch()
	{
		...
		/*
		//////////////////////////////Bonus///////////////////////////////////
		List<Movie> listOfMovies = s.getListMovies();
		System.out.println();
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>> BONUS TIME!! <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		System.out.println();
		
		
		//Display 5 movies that have the highest rating
		List<Movie> MoviesHighRating = s.findHighestRating(listOfMovies, 5);
		printResults("------------ This is Top 5 Highest Rating Movies ------------", MoviesHighRating, -1);
		System.out.println();
		
		
		//Display 5 movies that have the lowest rating
		List<Movie> MoviesLowRating = s.findLowestRating(listOfMovies, 5);
		printResults("------------ This is Top 5 Lowest Rating Movies ------------", MoviesLowRating, -1);
		System.out.println();
		
		
		//Display 5 movies of each tag that have the highest rating
		String[] allTag = {"Adventure","Animation","Children","Comedy","Fantasy","Drama","Romance",
						   "Action","Crime","Thriller","Horror","Sci-Fi","Mystery","IMAX","War","Musical",
						   "Documentary","Film-Noir","Western"};
		System.out.println("################# Top Movies In Each Tag #################");
		System.out.println();
		for(String Atag : allTag)
		{
			List<Movie> Tag_HighRating = s.findTagHighestRating(Atag);
			if(Tag_HighRating != null)
			{
				printResults("From " + Atag + " tag", Tag_HighRating, -1);
				System.out.println();
			}	
		} 
		*/
